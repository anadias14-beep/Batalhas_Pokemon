...
function App() {
  ...
  // 2. Filtra Pokémons elegíveis para a Arena Escolhida
  ...
  // Troca de Arena Limpa o Time se os Pokémons não forem permitidos
  ...
  const iniciarBatalha = () => {
    ...
    // O Rival também respeita os Pokémons da arena
  ...
  const ataqueJogador = (golpe) => {
    ...
    if (pRival.hpAtual === 0) {
      ...
      if (idxRival + 1 < 3) {
        ...
        setTela('fim');
        setPontos(pontos + 100); // Ganha Pontos
      ...
      const timer = setTimeout(() => {
        ...
        if (pJogador.hpAtual === 0) {
          ...
          if (idxJogador + 1 < 3) {
            ...
            setTela('fim');
            setPontos(Math.max(0, pontos - 50)); // Perde Pontos
  ...
  }, [turno, tela, idxJogador, idxRival, meuTime, timeRival, log, pontos]);
  ...
  if (tela === 'fim') {
    return (
      <div style={{ backgroundColor: ARENAS[arena].bg, minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h1 style={{ fontSize: '45px', color: mensagemFim.includes('VENCEU') ? '#2e7d32' : '#c62828' }}>{mensagemFim}</h1>
        <h2>Pontuação Atual: ⭐ {pontos}</h2>
  ...
  return (
    <div style={{ backgroundColor: ARENAS[arena].bg, minHeight: '100vh', padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontFamily: 'Arial' }}>Arena de Batalha Serverless</h1>
        <h2 style={{ backgroundColor: '#fff', padding: '10px 20px', borderRadius: '20px', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}>
          ⭐ Pontos: {pontos}
        </h2>
      </div>

      {/* Seleção de Arenas */}
      <div style={{ backgroundColor: 'rgba(255,255,255,0.9)', padding: '15px', borderRadius: '10px', marginBottom: '20px' }}>
